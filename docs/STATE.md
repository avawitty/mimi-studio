...
| **Tailor** | `/tailor` | shipped | Style Lab / Diagnostics aliased routes; **Owner Slide Templates** UI; 10 editorial calibration plates (incl. Used Context, contact sheet, material specimen, forecast drift); **IndexedDB local fallback** when Firestore free-tier daily read quota is exhausted | Wire Doll ↔ Rip handoff CTAs (`prd/chamber-loop-…`) |
