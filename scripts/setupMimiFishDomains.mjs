#!/usr/bin/env node
/**
 * Setup mimi.fish for Auth authorized domains (+ optional Vercel domain attach).
 *
 * Prerequisites:
 ...
 *
 * Usage:
 *   node scripts/setupMimiFishDomains.mjs
 *   node scripts/setupMimiFishDomains.mjs --project mimistudios
 *   VERCEL_TOKEN=… node scripts/setupMimiFishDomains.mjs --vercel-project mimi-studio
 */

import { execFileSync } from "node:child_process";
import { readFileSync, existsSync } from "node:fs";

const DOMAINS_TO_ADD = ["mimi.fish", "www.mimi.fish"];
...
async function ensureVercelDomain(projectName) {
  ...
  if (!token) {
    console.log("Vercel: skipped (no VERCEL_TOKEN). Add mimi.fish in Vercel → Project → Domains.");
    return null;
  }
  const headers = {
...
async function main() {
  ...
  console.log("\nNotes:");
  console.log("- CORS for https://mimi.fish is already in functions/src/index.ts.");
  console.log("- Share plates: mimi.fish/s/:zineId (same PublicZineSharePage as /s/).");
  console.log("- Shelves: mimi.fish/u/:handle → public plates linking to /s/:id.");
  console.log("- Attach mimi.fish in Vercel → Project → Domains if not already live.");
}
