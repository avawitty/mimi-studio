...
const DIAL_SIZE = 56; // diameter of the orbit button
const SPOKE_SIZE = 52; // diameter of each fanned-out spoke
const RADIUS = 108; // distance spokes sit from the dial center
...
export const FloatingDialNav: React.FC<FloatingDialNavProps> = ({
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (saved) {
        ...
        // clamp into the current viewport in case it resized
        setPos({
  ...
  // Which half of the screen is the dial in? Spokes fan toward center.
  ...
  // Compute an arc of angles facing the screen center.
  const spokeOffsets = useMemo(() => {
    const n = items.length;
    // Base direction (degrees): point the arc toward the center quadrant.
    // 0deg = right, 90 = down, 180 = left, 270 = up.
    let start: number;
    let end: number;
    if (center.onLeft && !center.onTop) {
      start = -80; end = 10; // bottom-left -> fan up/right
    } else if (!center.onLeft && !center.onTop) {
      start = 190; end = 280; // bottom-right -> fan up/left
    } else if (center.onLeft && center.onTop) {
      start = 10; end = 100; // top-left -> fan down/right
    } else {
      start = 80; end = 170; // top-right -> fan down/left
    }
    ...
    <div className="md:hidden" aria-hidden={false}>
      ...
      <motion.div
        ...
        onDragEnd={(_, info) => {
          ...
          // allow tap again after the drag settles
          setTimeout(() => {
      ...
      >
        ...
        {/* Optional full-index spoke (drawer) */}
        <AnimatePresence>
          {open && onOpenIndex && (
            <motion.button
              type="button"
        ...
        {/* The orbit button itself */}
        <button
          type="button"
          onClick={() => {
            if (draggingRef.current) return; // a drag just ended — ignore the click
            setOpen((v) => !v);
          }}
